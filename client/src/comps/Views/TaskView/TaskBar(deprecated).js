...
import { setChat, getChat, getCUser, setRefGroups, getRefGroups } from '../../../GraphQL/Cache';
import { createGroup } from '../../../GraphQL/Qur/Mutation';
import { user, glossaryStatus } from '../../../GraphQL/Qur/Query';
...
class Tasks extends Component {
  ...
  render() {
    ...
    const { getChat, getCUser, getRefGroups } = this.props;
                ...
                status.map((e,i)=>{
                  ...
                  return(
                    <div className="colName" key={"col"+e.id}>
                      ...
                      {cols[i].map((e,i)=>{
                        return(
                          <div key={"gr"+i} role="presentation" className={grid === e.id || getChat.id === e.id ? 'active list animated fadeIn' : 'list animated fadeIn'} onClick={()=>{this.changeGroup(e.id, e.name)}}>{e.name}</div>
                        )
                      })
                      }
                    </div>
