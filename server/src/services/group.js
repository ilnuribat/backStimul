...
module.exports = {
  assignedTo: function assignedTo(parent) {
  ...
  getMessages: async function getMessages(parent, { messageConnection }, { user }) {
    const { id } = parent;
  ...
  unreadCount: async function unreadCount({ id }, args, { user }) {
  ...
  getMembers: async function getMembers(group) {
    const { id } = group;
