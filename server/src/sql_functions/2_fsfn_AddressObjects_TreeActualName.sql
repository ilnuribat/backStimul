BEGIN
  TRANSACTION;
  DROP FUNCTION IF EXISTS fsfn_AddressObjects_TreeActualName (a_AOGUID VARCHAR(36), a_MaskArray VARCHAR(2) [ 10 ])
    CASCADE;
    /*****************************************************************************/
    /* Возвращает строку с полным названием адресообразующего элемента  */
    /*****************************************************************************/
    CREATE OR REPLACE FUNCTION fsfn_AddressObjects_TreeActualName (a_AOGUID VARCHAR(36) DEFAULT NULL,
    /* Идентификтор */
    /* адресообразующего  элемента */
    a_MaskArray VARCHAR(2) [ 10 ] DEFAULT '{TP,LM,LP,ST}'
    /* Массив масок, */
...
BEGIN
  SELECT
    INTO v_MinCurrStatus MIN(addrobj.currstatus)
  FROM
    "fias_AddressObjects" addrobj
  WHERE
    aoguid = a_AOGUID;
  OPEN cursor_AddressObjectTree FOR
    SELECT
      rtf_ShortTypeName,
      REPLACE(rtf_AddressObjectName, '  ', ' '),
      rtf_AOLevel,
      fsfn_AddressObjects_ObjectGroup (rtf_AOGUID)
    FROM
      fstf_AddressObjects_AddressObjectTree (a_AOGUID)
    ORDER BY
      rtf_AOLevel;
  v_TreeLeverCount: = 0;
  ...
  FETCH FIRST
FROM
  cursor_AddressObjectTree INTO v_ShortTypeName,
  v_AddressObjectName,
  v_AOLevel,
  v_ObjectGroup;
  WHILE FOUND LOOP
    ...
    IF v_ObjectGroup = c_CountryGroupValue AND c_CountryMask <@ a_MaskArray AND v_AOLevel = 0 THEN
      v_TreeAddressObjectName: = v_TreeAddressObjectName || CASE WHEN v_TreeAddressObjectName = '' THEN
        ''
      ELSE
        ', '
      END || v_AddressObjectName || ' ' || v_ShortTypeName;
ELSIF v_ObjectGroup = c_RegionGroupValue
  ...
  ELSE
    ', '
END || CASE WHEN UPPER(v_ShortTypeName)
  LIKE UPPER('%Респ%') THEN
  'Республика ' || v_AddressObjectName
ELSE
...
ELSIF v_ObjectGroup = c_CityGroupValue
  AND c_MainLocalityMask <@ a_MaskArray
  AND v_AOLevel = 4 THEN
  v_TreeAddressObjectName: = v_TreeAddressObjectName || CASE WHEN v_TreeAddressObjectName = '' THEN
    ''
  ELSE
    ', '
END || CASE WHEN UPPER(
  LEFT (v_AddressObjectName, 6 + LENGTH(v_ShortTypeName))) = 'ЗАТО ' || UPPER(TRIM(v_ShortTypeName)) || '.' THEN
v_AddressObjectName
ELSE
  v_ShortTypeName || ' ' || v_AddressObjectName
END;
ELSIF v_ObjectGroup = c_LocalityGroupValue
  AND c_DistrictMask <@ a_MaskArray
  AND v_AOLevel = 5 THEN
  v_TreeAddressObjectName: = v_TreeAddressObjectName || CASE WHEN v_TreeAddressObjectName = '' THEN
    ''
  ELSE
    ', '
...
ELSIF v_ObjectGroup = c_LocalityGroupValue
  ...
  ELSE
    ', '
END || v_ShortTypeName || ' ' || v_AddressObjectName;
...
COMMENT ON FUNCTION fsfn_AddressObjects_TreeActualName (a_AOGUID VARCHAR(36), a_MaskArray VARCHAR(2) [ 10 ])
