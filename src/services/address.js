...
const { ADDRESS_LEVELS } = require('../services/constants');
...
async function formAddress(rawAddress) {
  ...
  // Если адрес указан до дома, тут сохраняем до уровня улицы
  result.fiasId = result.house.fiasId ? result.street.fiasId : result.fiasId;

  result.parentChain = await getParentChain(result.fiasId);
